# APP CONTROL ARTEFACTS

Uploaded applications in APP CONTROL are described using an XML file. 
For APP CONTROL, an aplication is a hierarchy set of components.
...
A minimalist APP CONTROL <em>Hello World</em> ARTEFACT:
```xml
...
The ```father``` tag creates a dependency on a component. The App Control approach is based on the fact that child components have the knowledge of father(s).
...
<br>
The syntax of variables in App Control is: $ (variableName)
